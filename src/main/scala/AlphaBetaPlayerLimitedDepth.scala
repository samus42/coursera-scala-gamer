...
class AlphaBetaPlayerLimitedDepth extends NotifyingPlayer {
    ...
    override def bestmove(role: Role, state: MachineState) = {
        ...
        actions.maxBy(a => {
            val result = minscore(role, a, state, 0, 100, 0)
    ...
    def maxscore(role: Role, state: MachineState, alpha: Int, beta: Int, level: Int): Int = {
        def helper(actions: List[Move], curAlpha: Int): Int = {
            actions match {
                case Nil => curAlpha
                case head :: tail =>
                    val result = minscore(role, head, state, curAlpha, beta, level)
    ...
    def minscore(role: Role, action: Move, state: MachineState, alpha: Int, beta: Int, level : Int): Int = {
        ...
        def helper(actions: List[Move], curBeta: Int): Int = {
            actions match {
                ...
                case head :: tail =>
                    ...
                    val result = maxscore(role, newState, alpha, curBeta, level + 1)
